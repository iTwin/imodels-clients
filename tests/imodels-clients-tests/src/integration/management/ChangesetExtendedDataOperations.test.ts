...
describe("[Management] ChangesetExtendedDataOperations", () => {
  ...
  it("should return all items when querying collection", async () => {
    ...
    await assertCollection({
      ...
      isEntityCountCorrect: (count) => (count > 0 && count === testIModel.changesetExtendedData.length)
    });
  });
  ...
  it("should get valid changeset extended data when querying collection", async () => {
    ...
    const expectedChangesetExtendedData = testIModel.changesetExtendedData.find((x) => x.changesetIndex === actualChangesetExtendedData.changesetIndex);
    expect(actualChangesetExtendedData).to.exist;
    assertChangesetExtendedData({ actualChangesetExtendedData, expectedChangesetExtendedData: expectedChangesetExtendedData! });
  });
  ...
  it("should get changeset extended data by changeset index", async () => {
    ...
    // Act
    const changesetExtendedData = await iModelsClient.changesetExtendedData.getSingle(getSingleChangesetExtendedDataParams);

    // Assert
    assertChangesetExtendedData({ actualChangesetExtendedData: changesetExtendedData, expectedChangesetExtendedData });
  });

  it("should return error when querying non-existent changeset extended data", async () => {
    // Arrange
    const notExistingChangesetExtendedData = testIModel.changesetExtendedData.length + 1;
    const getSingleChangesetExtendedDataParams: GetSingleChangesetExtendedDataParams = {
      authorization,
      iModelId: testIModel.id,
    ...
    try {
      await iModelsClient.changesetExtendedData.getSingle(getSingleChangesetExtendedDataParams);
    } catch (e) {
      objectThrown = e;
    }
