...
describe("[Management] LockOperations", () => {
  ...
  it("should return paged results when querying released locks with continuation token", async () => {
    // Arrange
    const briefcase = await authoringClient.briefcases.acquire({
      authorization,
      iModelId: testIModelForWrite.id,
    ...
    const objectIds = ["0xb1", "0xb2", "0xb3", "0xb4", "0xb5", "0xb6"];
    await authoringClient.locks.update({
      authorization,
      iModelId: testIModelForWrite.id,
      briefcaseId: briefcase.briefcaseId,
      lockedObjects: [{ lockLevel: LockLevel.Exclusive, objectIds }],
    });
    ...
    await authoringClient.locks.update({
      ...
      briefcaseId: briefcase.briefcaseId,
      changesetId,
      lockedObjects: [{ lockLevel: LockLevel.None, objectIds }],
    });
    ...
    const getReleasedLockListParams: GetReleasedLockListParams = {
      authorization,
      iModelId: testIModelForWrite.id,
      urlParams: { afterChangesetIndex: 0, $top: top, $continuationToken: "" },
    };
    ...
    // Assert - iterator fetches all pages via continuation token
    const releasedLockArray = await toArray(releasedLocks);
    const totalObjectIds = releasedLockArray.reduce(
      (sum, lock) => sum + lock.objectIds.length,
      0
    );
    expect(totalObjectIds).to.be.greaterThan(0);
  });
});
