 ...
 *--------------------------------------------------------------------------------------------*/
import { iModelsRestClient } from "../iModelsRestClient";
...
export class iModelOperations extends iModelsRestClient {
  ...
  private async getList<TiModel>(params: RequestContextParam & { url: string, preferReturn: PreferReturn }): Promise<EntityCollectionPage<TiModel>> {
    const response = await this.sendGetRequest<iModelsResponse<TiModel>>({
      url: params.url,
      headers: this.getHeaders(params)
    });
    return {
